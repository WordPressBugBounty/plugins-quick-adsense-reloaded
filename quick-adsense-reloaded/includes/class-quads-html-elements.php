...
class QUADS_HTML_Elements {
	...
	public function textarea( $args = array() ) {
		...
		$output = '<span id="quads-' . sanitize_key( $args['name'] ) . '-wrap">';
			...
			$output .= '<textarea placeholder="' . esc_attr( $args['placeholder'] ) . '" name="' . esc_attr( $args['name'] ) . '" id="' . esc_attr( $args['id'] ) . '" class="' . $args['class'] . '"' . $disabled . '>' . esc_textarea( $args['value'] ) . '</textarea>';
